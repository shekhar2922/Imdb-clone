...
function Wishlist() {
  ...
  filteredMovies =
    curGenre == "All Genres"
      ? Favorites
      : Favorites.filter((movie) => genreids[movie.genre_ids[0]] == curGenre);
  // sorting
  if (rating == 1) {
  ...
  return (
    <>
      <div className="m-4 px-2 flex justify-center flex-wrap">
        {genres.map((genre) => (
          <button
            className={
              curGenre == genre
                ? "m-2 text-xs md:text-lg p-1 px-2 bg-blue-400 text-yellow-300 rounded-xl font-bold"
                : "m-2 text-xs md:text-lg p-1 px-2 bg-gray-400 hover:bg-blue-400 text-yellow-300 rounded-xl font-bold"
            }
            onClick={() => {
              setCurPage(1);
              setCurGenre(genre);
      ...
      <div className="text-center">
        <input
          ...
          placeholder="Search"
          className="border border-1 md:border-3 rounded-md text-center font-light border-gray-500 p-0.5 md:p-1 m-0.5 md:m-3"
        />
        <input
          type="number"
          value={rows}
